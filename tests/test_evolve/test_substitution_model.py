...
class TupleModelMotifProbFuncs(TestCase):
    def __init__():
        self.dinucs = ('TT', 'CT', 'AT', 'GT',
                  'TC', 'CC', 'AC', 'GC',
                  'TA', 'CA', 'AA', 'GA',
                  'TG', 'CG', 'AG', 'GG')
        self.nuc_probs = [('T', 0.1), ('C', 0.2), ('A', 0.3), ('G', 0.4)]
        self.dinuc_probs=[(m2+m1,p1*p2) for m1,p1 in self.nuc_probs for m2,p2 in self.nuc_probs]
        self.mat_indices = dict(
